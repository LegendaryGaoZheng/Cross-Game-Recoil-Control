...
/*
  -------------------------
    Legendary压枪助手 Legendary v2.4.6
  -------------------------
...
class LegendaryApp {
    ...
    __New() {
        ...
        this.CreateGUI()
        ; 启动时未打开“配置管理”GUI，ConfigListCtrl 还未创建
        if ObjHasOwnProp(this, "ConfigListCtrl")
        ...
        try {
            ...
            try {
                if PixelSearch(&FoundX, &FoundY, x1, y1, x2, y2, targetColor, this.ImageTolerance) {
                    ; 找到目标颜色，触发按键
                    SendInput this.TriggerKey
                    if this.ImageDebug {
                        if (A_TickCount - this._lastImageDebugTipTick > 250) {
                            this._lastImageDebugTipTick := A_TickCount
                            ToolTip "命中：" FoundX "," FoundY "  目标色=0x" Format("{:06X}", targetColor & 0xFFFFFF) "  容差=" this.ImageTolerance
                            SetTimer () => ToolTip(), -600
                        }
                    }
                    Sleep 50 ; 防止连续触发
                } else if this.ImageDebug {
    ...
    CreateGUI() {
        ...
        this.ED_Ctrl := this.MyGui.Add("CheckBox", "xm y+8", "启用辅助")
        this.ED_Ctrl.Value := this.ED
        this.ED_Ctrl.OnEvent("Click", this._cbCheckboxChanged)
        this.MyGui.Add("Button", "x+10 yp-3 w60", "配置").OnEvent("Click", this._cbOpenRecoilConfig)

        this.BreathHoldCtrl := this.MyGui.Add("CheckBox", "xm y+8", "启用屏息")
        ...
        this.MyGui.Add("Button", "x+10 yp-3 w60", "配置").OnEvent("Click", this._cbOpenBreathConfig)

        this.SemiAutoModeCtrl := this.MyGui.Add("CheckBox", "xm y+8", "启用半自动模式")
        ...
        this.MyGui.Add("Button", "x+10 yp-3 w60", "配置").OnEvent("Click", this._cbOpenSemiAutoConfig)

        this.Cut31Ctrl := this.MyGui.Add("CheckBox", "xm y+8", "启用31切枪")
        this.Cut31Ctrl.Value := this.Cut31Enabled ? 1 : 0
        this.Cut31Ctrl.OnEvent("Click", this._cbCut31Changed)
        this.MyGui.Add("Button", "x+10 yp-3 w60", "配置").OnEvent("Click", this._cbOpenCut31Config)

        this.ImageRecCtrl := this.MyGui.Add("CheckBox", "xm y+8", "图像识别自动触发")
        this.ImageRecCtrl.Value := this.ImageRecEnabled ? 1 : 0
        this.ImageRecCtrl.OnEvent("Click", this._cbImageRecChanged)
        this.MyGui.Add("Button", "x+10 yp-3 w60", "配置").OnEvent("Click", this._cbOpenImageRecConfig)

        this.MyGui.Add("Text", "xm y+14 w80", "")
        this.StatusTextCtrl := this.MyGui.Add("Text", "xm y+2", "状态：未启用")

        this.MyGui.Show "w380 h360"
    }
    ...
    OpenRecoilConfig(*) {
        ...
        this.RecoilConfigGui := Gui("+Owner" this.MyGui.Hwnd, "配置 - 启用辅助")
        ...
        this.RecoilConfigGui.Show("w460 h320")
    }
    ...
    OpenBreathConfig(*) {
        ...
        this.BreathConfigGui.Add("Text", "xm ym w80", "屏息键：")
        this.BreathHoldKeyCtrl := this.BreathConfigGui.Add("Edit", "x+5 yp-3 w120", this.breathHoldKey)
        this.BreathConfigGui.Add("Button", "xm y+18 w100", "应用设置").OnEvent("Click", this._cbApplySettings)
        this.BreathConfigGui.Show("w260 h110")
    }
    ...
    OpenSemiAutoConfig(*) {
        ...
        this.SemiAutoConfigGui.Add("Text", "xm ym w220", "该功能无额外参数。")
        this.SemiAutoConfigGui.Show("w260 h90")
    }
    ...
    OpenImageRecConfig(*) {
        ...
        local useTransCtrl := this.ImageRecConfigGui.Add("CheckBox", "x+5 yp-3", "启用")
        useTransCtrl.Value := this.ImageUseTrans ? 1 : 0
        useTransCtrl.OnEvent("Click", (c, *) => (this.ImageUseTrans := c.Value ? 1 : 0, this.SaveSettings()))
        local transEdit := this.ImageRecConfigGui.Add("Edit", "x+8 yp w120", this.ImageTransColor)
        transEdit.OnEvent("Change", (c, *) => (this.ImageTransColor := Trim(c.Value), this.SaveSettings()))
        ...
        this.ImageRecConfigGui.Show("w500 h470")
    }
    ...
    OpenConfigManager(*) {
        ...
        this.ConfigManagerGui.Add "Text", "xm ym w80", "已存配置："
        this.ConfigListCtrl := this.ConfigManagerGui.Add("DropDownList", "x+5 yp-3 w150")
        this.ConfigManagerGui.Add("Button", "x+5 yp w70", "加载选中").OnEvent("Click", this._cbLoadSelectedConfig)
        this.ConfigManagerGui.Add("Button", "x+5 yp w60", "刷新").OnEvent("Click", this._cbRefreshConfigList)

        this.ConfigManagerGui.Add "Text", "xm y+12 w80", "配置名称："
        this.ConfigNameCtrl := this.ConfigManagerGui.Add("Edit", "x+5 yp-3 w150")
        this.ConfigManagerGui.Add("Button", "x+5 yp w60", "保存").OnEvent("Click", this._cbSaveCurrentConfig)
        this.ConfigManagerGui.Add("Button", "x+5 yp w60", "删除").OnEvent("Click", this._cbDeleteSelectedConfig)

        this.ConfigManagerGui.Show("w420 h150")
        this.RefreshConfigList()
    }
