...
/*
  -------------------------
    Legendary压枪助手 Legendary v2.3.9
  -------------------------
  基于 v2.3.4，新增「31切枪」功能。
  v2.3.4：应用设置同步触发键、退出清理、配置名过滤、ToolTip、弹道预览、帮助窗口、热键冲突提示。
  v2.3.7：31切枪（滚轮下划触发），等待(可调)→3→10ms→1。
  v2.3.9：优化滚轮透传、压枪循环节流、异常安全、参数校验实现。
  配置与 v2.2.6 共用 AutoFire.ini。
  */
Persistent
global scriptVersion := "v2.3.9"
...
global assistantEnabled := true   ; 总开关，热键切换
...
global ED := 1
global Cut31Enabled := 1
global Cut31Interval := 60
...
global TriggerKeyCtrl, Cut31Ctrl, Cut31IntervalCtrl
global TrajectoryPicCtrl, TrajectorySlopeCtrl, TrajectoryTitleCtrl
global TrajectoryDrawHwnd
global _TrajOldWndProc := 0
global _TrajWndProcCallback := 0
...
A_IconTip := "Legendary压枪助手 " scriptVersion
...
; -------------------------------
HotkeyToggle(*) {
    global assistantEnabled
    ; 总开关：仅切换 assistantEnabled，不改动各功能勾选
    assistantEnabled := !assistantEnabled
    UpdateStatusDisplay()
    SyncComboHotkey()
    SyncSideHotkey()
    SyncCut31Hotkey()
    ToolTip assistantEnabled ? "总开关已启用" : "总开关已禁用（已勾选功能暂停）"
...
SyncCut31Hotkey() {
    global Cut31Enabled, assistantEnabled
    try {
        ; 勾选31切枪 且 总开关启用 时捕获滚轮，否则滚轮传给游戏
        if Cut31Enabled && assistantEnabled
            Hotkey "~WheelDown", Do31Cut, "On"
        else
            Hotkey "~WheelDown", Do31Cut, "Off"
    } catch {
    }
}

Do31Cut(*) {
    global Cut31Enabled, Cut31Interval, assistantEnabled
    GetCurrentValues()
    if !Cut31Enabled || !assistantEnabled
        return
    local ms := Integer(Cut31Interval)
    if ms < 10
        ms := 10
    if ms > 2000
        ms := 2000
...
SideKeyPressed(*) {
    global breathHold, breathHoldKey, TriggerSideKey, assistantEnabled
    if !breathHold || !assistantEnabled
        return
    if breathHold {
...
CreateGUI() {
    ...
    global breathHold, breathHoldKey, semiAutoMode, ED, Cut31Enabled, Cut31Interval
    global TriggerKeyCtrl, Cut31Ctrl, Cut31IntervalCtrl
    global TrajectoryPicCtrl, TrajectorySlopeCtrl, TrajectoryTitleCtrl, TrajectoryDrawHwnd
    global _TrajOldWndProc, _TrajWndProcCallback, TRAJ_WIDTH, TRAJ_HEIGHT

    MyGui := Gui("+Resize", "Legendary压枪助手 " scriptVersion)
    MyGui.OnEvent("Close", GuiClose)
    MyGui.OnEvent("Escape", GuiEscape)
    MyGui.OnEvent("Size", GuiSize)
    MyGui.SetFont("s10", "Microsoft YaHei")

    MyGui.Add("Button", "x388 y5 w90 h25", "操作指南").OnEvent("Click", ShowHelpGuide)
    ...
    MyGui.Add("Text", "xm y+10 w80 cBlue", "")
    Cut31Ctrl := MyGui.Add("CheckBox", "x+5 yp-3", "31切枪(滚轮下)")
    Cut31Ctrl.Value := Cut31Enabled ? 1 : 0
    Cut31Ctrl.OnEvent("Click", Cut31Changed)
    MyGui.Add("Text", "x+5 yp+3 w70", "间隔(ms)：")
    Cut31IntervalCtrl := MyGui.Add("Edit", "x+2 yp-3 w50 Number", Cut31Interval)
    MyGui.Add("Text", "x+5 yp+3 w80 cGray", "10-2000")
    ...
    TrajectoryTitleCtrl := MyGui.Add("Text", "x388 y35 w120 Center", "弹道预览")
    TrajectoryPicCtrl := MyGui.Add("Text", "x388 y+2 w" TRAJ_WIDTH " h" TRAJ_HEIGHT " Border")
    ...
    TrajectorySlopeCtrl := MyGui.Add("Text", "x388 y+2 w120 Center cGray", "垂直: 0  横向: 0")

    MyGui.Show "w540 h620"
    UpdateTrajectoryPreview()
}

Cut31Changed(ctrlObj, *) {
    global Cut31Enabled
    Cut31Enabled := ctrlObj.Value ? 1 : 0
    SyncCut31Hotkey()
    ToolTip Cut31Enabled ? "31切枪已启用（滚轮下划）" : "31切枪已关闭"
    SetTimer () => ToolTip(), -1500
}

GuiSize(GuiObj, MinMax, Width, Height) {
    global TrajectoryTitleCtrl, TrajectoryPicCtrl, TrajectorySlopeCtrl, TRAJ_WIDTH, TRAJ_HEIGHT
    if MinMax = -1 || !IsSet(TrajectoryPicCtrl) || !TrajectoryPicCtrl
        return
    local trajX := 388
    if (Width < trajX + TRAJ_WIDTH + 20) {
        try {
            TrajectoryTitleCtrl.Visible := false
            TrajectoryPicCtrl.Visible := false
            TrajectorySlopeCtrl.Visible := false
        } catch {
        }
    } else {
        try {
            TrajectoryTitleCtrl.Visible := true
            TrajectoryPicCtrl.Visible := true
            TrajectorySlopeCtrl.Visible := true
            TrajectoryTitleCtrl.Move(trajX, 35)
            TrajectoryPicCtrl.Move(trajX, 55)
            TrajectorySlopeCtrl.Move(trajX, 55 + TRAJ_HEIGHT + 2)
        } catch {
        }
    }
}
...
UpdateTrajectoryPreview(*) {
    global TrajectoryDrawHwnd, TrajectorySlopeCtrl, RecoilForceCtrl, HorizontalRecoilCtrl, RecoilForce, HorizontalRecoil
    try {
...
ShowHelpGuide(*) {
    ...
    HelpGui.OnEvent("Size", (GuiObj, MinMax, W, H) => SizeHelpEdit(GuiObj, MinMax, W, H, HelpText))
...
GetCurrentValues() {
    global FireRate, RecoilForce, HorizontalRecoil, HorizontalPattern
    global breathHold, breathHoldKey, semiAutoMode, ED, TriggerSideKey, Cut31Enabled, Cut31Interval
    global FireRateCtrl, RecoilForceCtrl, HorizontalRecoilCtrl, HorizontalPatternCtrl
    global BreathHoldCtrl, BreathHoldKeyCtrl, SemiAutoModeCtrl, ED_Ctrl, TriggerKeyCtrl, Cut31Ctrl, Cut31IntervalCtrl
    ...
    ED := ED_Ctrl.Value
    Cut31Enabled := Cut31Ctrl.Value ? 1 : 0
    local ival := Trim(Cut31IntervalCtrl.Value)
    Cut31Interval := (ival != "" && IsNumber(ival)) ? Integer(ival) : 60
    if IsSet(TriggerKeyCtrl) && TriggerKeyCtrl {
        local label := TriggerKeyCtrl.Text
        if InStr(label, "XButton2")
            TriggerSideKey := "XButton2"
        else if InStr(label, "XButton1")
            TriggerSideKey := "XButton1"
...
UpdateGUIDisplay() {
    ...
    ED_Ctrl.Value := ED
    Cut31Ctrl.Value := Cut31Enabled ? 1 : 0
    Cut31IntervalCtrl.Value := Cut31Interval
    if IsSet(TriggerKeyCtrl) && TriggerKeyCtrl
        TriggerKeyCtrl.Choose(TriggerSideKey = "XButton1" ? 2 : (TriggerSideKey = "RButton" ? 3 : 1))
}

UpdateStatusDisplay() {
    global ED, assistantEnabled, semiAutoMode, HorizontalPattern, Cut31Enabled
    global StatusTextCtrl
    local master := assistantEnabled ? "总开" : "总关"
    local mode := semiAutoMode ? "半自动" : "全自动"
    local hMode := (HorizontalPattern = 1) ? "左右交替" : "固定补偿"
    local cut31 := Cut31Enabled ? " 31✓" : ""
    StatusTextCtrl.Text := "状态：" master " (" mode "，横向：" hMode cut31 ")"
}
...
CreateDefaultConfig() {
    ...
    try {
        ...
        IniWrite "1", configFile, "Settings", "Cut31Enabled"
        IniWrite "60", configFile, "Settings", "Cut31Interval"
    } catch {
...
LoadSettings() {
    ...
    try {
        ...
        Cut31Enabled := Integer(IniRead(configFile, "Settings", "Cut31Enabled", 1)) != 0
        Cut31Interval := Integer(IniRead(configFile, "Settings", "Cut31Interval", 60))

        FireRate := ClampInt(FireRate, 100, 2000, 600)
        RecoilForce := ClampInt(RecoilForce, 0, 30, 5)
        HorizontalRecoil := ClampInt(HorizontalRecoil, -15, 15, 0)
        HorizontalPattern := ClampInt(HorizontalPattern, 0, 1, 0)
        Cut31Interval := ClampInt(Cut31Interval, 10, 2000, 60)
    } catch as err {
...
SaveSettings() {
    ...
    try {
        ...
        IniWrite (Cut31Enabled ? "1" : "0"), configFile, "Settings", "Cut31Enabled"
        IniWrite Cut31Interval, configFile, "Settings", "Cut31Interval"
    } catch {
...
ApplySettings(*) {
    global HotkeyCC, HotkeyCtrl, TriggerSideKey
    global FireRate, RecoilForce, HorizontalRecoil, HorizontalPattern, Cut31Interval
    local oldHotkey := HotkeyCC
    GetCurrentValues()
    if (HotkeyCC = TriggerSideKey) {
...
SaveCurrentConfig(*) {
    global configFile
    global FireRate, RecoilForce, HorizontalRecoil, HorizontalPattern
    global HotkeyCC, breathHold, breathHoldKey, semiAutoMode, ED, TriggerSideKey, Cut31Enabled, Cut31Interval
    global ConfigNameCtrl
    GetCurrentValues()
    local rawName := Trim(ConfigNameCtrl.Value)
    local configName := SanitizeConfigName(rawName)
    if configName = "" {
    ...
    try {
        ...
        IniWrite (Cut31Enabled ? "1" : "0"), configFile, section, "Cut31Enabled"
        IniWrite Cut31Interval, configFile, section, "Cut31Interval"
        RefreshConfigList()
...
LoadSelectedConfig(*) {
    ...
    global Cut31Enabled, Cut31Interval
    ...
    try {
        ...
        Cut31Enabled := Integer(IniRead(configFile, section, "Cut31Enabled", 1)) != 0
        Cut31Interval := Integer(IniRead(configFile, section, "Cut31Interval", 60))
...
DeleteSelectedConfig(*) {
    ...
    if configName != ""
        configToDelete := SanitizeConfigName(configName)
    else if configList != ""
...
GuiClose(*) {
    global TrajectoryDrawHwnd, _TrajOldWndProc, _TrajWndProcCallback
    if (TrajectoryDrawHwnd && _TrajOldWndProc) {
        try
            DllCall("SetWindowLongPtr", "Ptr", TrajectoryDrawHwnd, "Int", -4, "Ptr", _TrajOldWndProc, "Ptr")
        catch {
        }
    }
    if _TrajWndProcCallback {
        try
            CallbackFree(_TrajWndProcCallback)
        catch {
        }
    }
    ExitApp
